...
def get_config(path: str, opts: Optional[list] = None) -> Tuple[DictConfig, str]:
    ...
    """
    # habitat_opts = [opt for opt in opts if "habitat" in opt]
    # other_opts = [opt for opt in opts if "habitat" not in opt]
    
    # config = get_habitat_config(path, habitat_opts)
    ...
    with read_write(config):
        ...
        # for i, opt in enumerate(other_opts):
        #     if opt == "config.dino.score_thresh":
        #         config.dino.score_thresh = other_opts[i+1]
        #     if opt == "config.dump_location":
        #         config.dump_location = other_opts[i+1]
