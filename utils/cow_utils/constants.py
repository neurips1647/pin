...
TEST_ROOM_IDS = [f"_test-challenge{i}_{j}" for i in range(1, 6) for j in range(1, 3)]
...
ACTION_SEQUENCES = (
    ...
    (
        ...
        (ROTATION_DEG * 6, FORWARD_M),
    ),
)

ROTATION_MATRICIES = [
    R.from_euler("y", a[-1][0], degrees=True).as_matrix() for a in ACTION_SEQUENCES
]
...
THOR_OBJECT_TYPES_MAP = {
    THOR_OBJECT_TYPES[i]: THOR_OBJECT_TYPES_CLIP[i]
    for i in range(len(THOR_OBJECT_TYPES))
}
...
GPT_HABITAT_OBJECT_TYPES_CLIP = [
    ...
    "bathtub in a bathroom or a master suite",
