...
from utils.cow_utils.src.models.agent import Agent
from utils.cow_utils.src.models.exploration.frontier_based_exploration import (
...
from utils.cow_utils.src.models.agent_mode import AgentMode
from threadpoolctl import threadpool_limits
import numpy as np

from utils.cow_utils.src.models.exploration.frontier_based_exploration import VoxelType
...
class AgentFbe(Agent, ABC):
    ...
    def __init__(
        ...
        if (360 - int(fov)) % self.rotation_degrees == 0:
            self.max_rotation_count = (360 - int(fov)) / self.rotation_degrees
        else:
            self.max_rotation_count = math.ceil((360 - int(fov)) / self.rotation_degrees)
    ...
    def act(self, observations, debug=False):
        ...
        with threadpool_limits(limits=1):
            ...
            # with threadpool_limits(limits=1):
            # update map
            self.fbe.update_map(observations, attention, self.last_action, debug, self.debug_dir, self.timesteps)
    ...
    def localize_object(self, observations) -> Tuple[int, float]:
        ...
        if "object_goal" in observations:
            return self.clip_module(img_tensor, observations["object_goal"])
        elif "pers_goal" in observations:
            return self.clip_module(img_tensor, observations["pin_goal"], observations["pers_goal"])
        else:
