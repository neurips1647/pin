...
from models.navigator.dino_navigator import EfficientSegmentAnything

# For debugging input and output maps - shows matplotlib visuals
debug_maps = False


class SparseCategorical2DSemanticMapModule(nn.Module):
    def __init__(
        self,
        device: torch.device,
        ...
        self.semantic_map_size_cm = map_size_cm
        self.global_downscaling = global_downscaling
        self.semantic_map_size = self.semantic_map_size_cm // self.resolution

        if evaluate_instance_tracking:
            num_channels += max_instances + 1

        self.semantic_map = [None for _ in range(self.num_environments)]
...
class Categorical2DSemanticMapModuleMultiON(nn.Module):
    """
    ...
    def __init__(
        ...
        mask_similarity_threshold: float = 0.0,
        max_distance_aggregation: float = 2.0,
    ):
        """
        Arguments:
        ...
        # )
        # self.detector_model = Mask2FormerForUniversalSegmentation.from_pretrained(
        #     "facebook/mask2former-swin-base-coco-panoptic"
        # )
        self.detector_model = EfficientSegmentAnything("data/weights/efficient_sam/efficient_sam_vits.pt.zip")
        ...
        self.max_distance_aggregation = max_distance_aggregation

    @torch.no_grad()
    def forward(
        self,
        seq_obs: Tensor,
        seq_obs_features: Tensor,
        ...
        for t in range(sequence_length):
            ...
            objects_map = self._update_objects_map(
                seq_obs[:, t],
                seq_obs_features[:, t],
                seq_pose_delta[:, t],
                objects_map,
    ...
    def _update_objects_map(
        ...
        """Update the objects map with the current observation."""
        # TODO make it batched
        device = obs.device
        rgb = obs[0, :3, :, :].float()
        self.detector_model.set_device(device)
        # inputs = self.detector_processor(images=rgb, return_tensors="pt").to(device)
        # with torch.no_grad():
        #     outputs = self.detector_model(**inputs)
        # masks_logits = outputs.masks_queries_logits
        # # TODO extract masks from logits here
        # masks_scale = np.array(masks_logits.shape[-2:]) / np.array(rgb.shape[-2:])
        # masks = self.detector_processor.post_process_instance_segmentation(outputs)[0]
        # masks, infos = masks["segmentation"], masks["segments_info"]
        masks = self.detector_model.run_everything(rgb)
        final_mask_size = tuple((np.array(rgb.shape[-2:]) / self.du_scale).astype(int))
        masks = F.interpolate(
            masks.unsqueeze(0).unsqueeze(0), size=final_mask_size, mode="nearest"
        )[0][0]
        masks = np.rint(masks)
        # filtered_masks, mask_positions = [], []
        # num_instances = int(masks.max().item() + 1)
        # for i in range(num_instances):
        #     if infos[i]["score"] > self.mask_confidence_threshold:
        #         curr_mask = masks == i
        #         mask_xy = curr_mask.nonzero()
        #         mask_xy = mask_xy * (
        #             np.array(rgb.shape[-2:]) / np.array(curr_mask.shape[-2:])
        #         )
        #         mean_mask_xy = np.rint(mask_xy.mean(dim=0))
        #         filtered_masks.append(curr_mask.clone())
        #         mask_positions.append(mean_mask_xy)

        # TODO project mask_pos to 2D map
        depth = obs[:, 3, :, :].float()
        depth[depth > self.max_depth] = 0

        new_instances_positions = []
        for idx, (x, y) in enumerate(mask_positions):
            curr_mask = (
                F.interpolate(
                    filtered_masks[idx].float().unsqueeze(0).unsqueeze(0),
                    size=depth.shape[-2:],
                    mode="nearest",
                )[0][0]
                > 0.5
            )
            curr_depth = depth * curr_mask.to(device)
            point_cloud_t = du.get_point_cloud_from_z_t(
                # depth[0:1, int(x) : int(x) + 1, int(y) : int(y) + 1],
                curr_depth,
                # depth,
                self.camera_matrix,
                device,
            ...
            y1 = self.local_map_size_cm // (self.xy_resolution * 2)
            y2 = y1 + self.vision_range
            agent_view[:, 0, y1:y2, x1:x2] = fp_exp_pred
            current_pose = pu.get_new_pose_batch(prev_pose.clone(), pose_delta)
            st_pose = current_pose.clone().detach()
            st_pose[:, :2] = -(
                (
            ...
            st_pose[:, 2] = 90.0 - (st_pose[:, 2])
            rot_mat, trans_mat = ru.get_grid(st_pose, agent_view.size(), obs.dtype)
            rotated = F.grid_sample(agent_view, rot_mat, align_corners=True)
            translated = F.grid_sample(rotated, trans_mat, align_corners=True)

            # Clamp to [0, 1] after transform agent view to map coordinates
            translated = torch.clamp(translated, min=0.0, max=1.0)
            mean_obj_xy = translated[0,0].nonzero().float().mean(0)
            global_pos_xy = mean_obj_xy + origins[0, :2]
            new_instances_positions.append(global_pos_xy.cpu()/self.xy_resolution)

        # returns a set of features (one for each object in the scene)
        masks = masks.unsqueeze(0) # since we are considering only one mask
        semantic_features = self._extract_objects_features(masks, obs_features)
        semantic_features = torch.rand((num_instances, 768))  # TODO remove this line when the above line is implemented

        for pos, feat in zip(new_instances_positions, semantic_features):
            feat = feat.unsqueeze(0)
            feat = feat / feat.norm(dim=-1, keepdim=True)
            if objects_map[0] is None:
                objects_map[0] = {pos: feat}
            else:
                for p, saved_feat in objects_map[0].items():
                    if np.linalg.norm(p - pos) < self.max_distance_aggregation:
                        for idx, sf in enumerate(saved_feat):
                            if len(sf.shape) == 1:
                                sf = sf.unsqueeze(0)
                            sim = torch.einsum('nc,mc->nm', sf, feat)
                            if sim > self.mask_similarity_threshold:
                                merged_feats = torch.stack((sf, feat)).mean(dim=0)
                                objects_map[0][p][idx] = merged_feats
                                break
                        else:
                            objects_map[0][pos].append(feat)
                            break
                else:
                    objects_map[0][pos] = feat
        return objects_map
    ...
    @torch.no_grad()
    def _extract_objects_features(self, masks, obs_features):
        b, h, w, c = obs_features.shape
        seg_masks = masks.unsqueeze(1).float().to(obs_features.device)
        seg_masks = interpolate(seg_masks, size=(h, w), mode='bilinear', align_corners=False)
        obs_features = obs_features.permute(0, 3, 1, 2)
        return (obs_features * seg_masks).sum(dim=(2, 3)) / (seg_masks.sum(dim=(2, 3)) + 1e-6)


class Categorical2DSemanticMapStateMultiON:
    """
    ...
    def __init__(
        ...
        self.objects_map = [None for _ in range(self.num_environments)]
    ...
    def get_objects_map(self, e) -> dict:
        return self.objects_map[e]
