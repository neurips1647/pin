...
class MultiONSuccess(Measure):
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        task.measurements.check_measure_dependencies(self.uuid, [SubSuccess.cls_uuid])
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        subsuccess = task.measurements.measures[SubSuccess.cls_uuid].get_metric()
...
class SubSuccess(Measure):
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        ...
        if (
            ...
            and distance_to_subgoal <= success_distance
        ):
            self._metric = 1
            task.current_goal_index += 1
            task.foundDistance = distance_to_subgoal
            if task.current_goal_index < len(episode.goals):
                task.measurements.measures[DistanceToCurrGoal.cls_uuid].update_metric(
                    *args, episode=episode, task=task, **kwargs
                )
        else:
            self._metric = 0
...
class CatSuccess(Measure):
    ...
    def reset_metric(
        ...
        task.measurements.check_measure_dependencies(
            self.uuid, [DistanceToCatGoal.cls_uuid]
        )
        self.update_metric(*args, episode=episode, task=task, **kwargs)

    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        distance_to_catgoal = task.measurements.measures[
            DistanceToCatGoal.cls_uuid
        ].get_metric()
        ...
        if (
            ...
            and distance_to_catgoal <= success_distance
        ):
            self._metric = 1.0
        else:
            self._metric = 0.0
...
class OracleSubSuccess(Measure):
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        ...
        if distance_to_subgoal <= success_distance:
            self._metric = 1
            task.current_goal_index += 1
            task.foundDistance = distance_to_subgoal
        else:
...
class PercentageSuccess(Measure):
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        sub_success = task.measurements.measures[SubSuccess.cls_uuid].get_metric()
...
@registry.register_measure
class CatSPL(Measure):
    r"""Categorical SPL (Success weighted by Path Length)"""
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        task.measurements.check_measure_dependencies(
            self.uuid, [DistanceToGoal.cls_uuid, CatSuccess.cls_uuid]
        )
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any, **kwargs: Any):
        ep_success = task.measurements.measures[CatSuccess.cls_uuid].get_metric()
...
class MSPL(Measure):
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        ...
        for goal_number in range(
            len(episode.goals)
        ):  # Find distances between successive goals and keep adding them
            if goal_number == 0:
                ...
                tmp = self._sim.geodesic_distance(
                    episode.goals[goal_number - 1].viewpoints[0],
                    episode.goals[goal_number].viewpoints,
                )
                if math.isinf(tmp):
                    self._start_end_episode_distance += self._euclidean_distance(
                        episode.start_position, episode.goals[0].viewpoints
...
class PSPL(Measure):
        ...
        for goal_number in range(
            ...
            if goal_number == 0:
                ...
                if math.isinf(tmp):
                    ...
                    self._start_end_episode_distance += tmp
                self._start_subgoal_episode_distance.append(
                    self._start_end_episode_distance
                )
            else:
                tmp = self._sim.geodesic_distance(
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        current_position = self._sim.get_agent_state().position.tolist()
        ep_percentage_success = task.measurements.measures[
            PercentageSuccess.cls_uuid
        ].get_metric()
        ...
        if ep_percentage_success > 0:
            self._metric = ep_percentage_success * (
                ...
                / max(
                    self._start_subgoal_agent_distance[-1],
                    self._start_subgoal_episode_distance[task.current_goal_index - 1],
                )
            )
        else:
    ...
    def reset_metric(self, *args: Any, episode, task, **kwargs: Any):
        ...
        for goal_number in range(
            ...
            if goal_number == 0:
                ...
                self._start_end_episode_distance += self._sim.geodesic_distance(
                    episode.goals[goal_number - 1][0].position,
                    episode.goals[goal_number][0].position,
                )
        self._agent_episode_distance = 0.0
        self._metric = None
        task.measurements.check_measure_dependencies(self.uuid, [Success.cls_uuid])
    ...
    def update_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        if self._config.distance_to == "POINT":
            ...
            if math.isinf(distance_to_target):
                distance_to_target = self._sim.geodesic_distance(
                    self._sim.pathfinder.snap_point(current_position),
                    self._sim.pathfinder.snap_point(episode.goals[0].position),
                    episode,
                )
...
@registry.register_measure
class DistanceToCatGoal(Measure):
    """The measure calculates a distance towards the goal."""

    cls_uuid: str = "distance_to_cat_goal"
    ...
    def reset_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        if self._config.distance_to == "VIEW_POINTS":
            ...
            goal_category = episode.goals[0].object_category
            self.goal_positions = [goal.position for goal in episode.goals]
            self.goal_positions += [
                goal.position
                for goal in episode.distractors
                if goal.object_category == goal_category
            ]
        self.update_metric(*args, episode=episode, **kwargs)
    ...
    def update_metric(self, episode, *args: Any, **kwargs: Any):
        ...
        if self._config.distance_to == "POINT":
            distance_to_target = min([self._sim.geodesic_distance(
                current_position, position) for position in self.goal_positions])
            if math.isinf(distance_to_target):
                distance_to_target = min([self._sim.geodesic_distance(
                    list(self._sim.pathfinder.snap_point(current_position)),
                    list(self._sim.pathfinder.snap_point(position)), episode) for position in self.goal_positions])
        elif self._config.distance_to == "VIEW_POINTS":
            distance_to_target = self._sim.geodesic_distance(
                current_position, self._episode_view_points
            )
        ...
        if self._config.distance_to == "POINT":
            ...
            if task.current_goal_index >= len(episode.goals):
                ...
                if math.isinf(distance_to_target):
                    distance_to_target = self._euclidean_distance(
                        current_position,
                        episode.goals[task.current_goal_index].viewpoints[0],
                    )
            for goal_number in range(task.current_goal_index, len(episode.goals) - 1):
                tmp = self._sim.geodesic_distance(
                    episode.goals[goal_number].viewpoints[0],
                    episode.goals[goal_number + 1].viewpoints,
                )
                if math.isinf(tmp):
...
class Ratio(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        #     )
        self._metric = (
            initial_geodesic_distance_to_target / initial_euclidean_distance_to_target
        )
...
class RawMetrics(Measure):
    ...
    def update_metric(self, *args: Any, episode, task: EmbodiedTask, **kwargs: Any):
        ...
        distance_to_curr_subgoal = task.measurements.measures[
            DistanceToCurrGoal.cls_uuid
        ].get_metric()
        ep_sub_success = task.measurements.measures[SubSuccess.cls_uuid].get_metric()
...
class DistanceToCurrGoal(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        ...
        if self._config.distance_to == "VIEW_POINTS":
            ...
            #    for view_point in goal.view_points
            # ]
            self._subgoal_view_points = episode.goals[
                task.current_goal_index
            ].viewpoints
        self.update_metric(*args, episode=episode, task=task, **kwargs)
