...
class PointNavDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional["DictConfig"] = None) -> None:
        ...
        for ep in self.episodes:
            if not os.path.isfile(ep.scene_id):
                new_scene_id = ep.scene_id.replace("hm3d_v0.2/", "hm3d/")
                new_scene_dataset_config = ep.scene_dataset_config.replace("hm3d_v0.2/", "hm3d/")
                if os.path.isfile(new_scene_id):
                    ep.scene_id = new_scene_id
                    ep.scene_dataset_config = new_scene_dataset_config
                else:
                    new_scene_id = ep.scene_id.replace("hm3d/", "hm3d_v0.2/")
                    new_scene_dataset_config = ep.scene_dataset_config.replace("hm3d/", "hm3d_v0.2/")
                    if not os.path.isfile(new_scene_id):
                        raise FileNotFoundError(f"Scene not found: {ep.scene_id}")
                    ep.scene_id = new_scene_id
                    ep.scene_dataset_config = new_scene_dataset_config
