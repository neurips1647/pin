...
@dataclass
class PINGoalSensorConfig(LabSensorConfig):
    type: str = "PINGoalSensor"
    goal_spec: str = "TASK_CATEGORY_ID"
    goal_spec_max_val: int = 50
    H: int = 960
    W: int = 1280
    object_images_path: str = "data/object_images"
...
@dataclass
class GTEgoMapConfig(LabSensorConfig):
    type: str = "GTEgoMap"
    map_size: int = 101
    map_scale: float = 0.05
    max_sensor_range: float = 3.25
    height_thresh: List[float] = field(default_factory=lambda: [0.2, 1.5])
...
@dataclass
class MultiONSuccessMeasurementConfig(MeasurementConfig):
    type: str = "MultiONSuccess"
    success_distance: float = 0.2
...
@dataclass
class CatSuccessMeasurementConfig(MeasurementConfig):
    type: str = "CatSuccess"
    success_distance: float = 0.2
...
@dataclass
class CatSPLMeasurementConfig(MeasurementConfig):
    type: str = "CatSPL"
...
@dataclass
class DistanceToCatGoalMeasurementConfig(MeasurementConfig):
    type: str = "DistanceToCatGoal"
    distance_to: str = "POINT"
...
class TaskConfig(HabitatBaseConfig):
    ...
    objects_path: str = ""
    objects_images_path: str = "data/images"
    include_distractors: bool = False
    exclude_cat_distractors: bool = False
...
cs.store(
    package="habitat.task.lab_sensors.pin_goal_sensor",
    group="habitat/task/lab_sensors",
    name="pin_goal_sensor",
    node=PINGoalSensorConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.lab_sensors.gt_ego_map",
    group="habitat/task/lab_sensors",
    name="gt_ego_map",
    node=GTEgoMapConfig,
)
...
cs.store(
    package="habitat.task.measurements.distance_to_cat_goal",
    group="habitat/task/measurements",
    name="distance_to_cat_goal",
    node=DistanceToCatGoalMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.multion_success",
    group="habitat/task/measurements",
    name="multion_success",
    node=MultiONSuccessMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.cat_success",
    group="habitat/task/measurements",
    name="cat_success",
    node=CatSuccessMeasurementConfig,
)
cs.store(
...
cs.store(
    package="habitat.task.measurements.cat_spl",
    group="habitat/task/measurements",
    name="cat_spl",
    node=CatSPLMeasurementConfig,
)
cs.store(
